  ...
  return (
    <section id='home' className='bg-white text-black dark:bg-slate-900 dark:text-white lg:px-20 lg:py-0 px-6 py-16 text-center gap-5 lg:text-start flex lg:flex-row flex-col-reverse justify-between lg:gap-20 items-center'>
      <div className='h-full lg:py-32 flex flex-col justify-center lg:items-start items-center'>
        ...
        <h2 data-aos='fade-down' itemProp="jobTitle" className='text-[18px] dark:text-gray-400 font-semibold mb-6 leading-normal'>
          ...
          <div className='flex items-center justify-center'>
            <div className='flex space-x-4'>
              {social.map((social, index) => (
                <a key={index} target="_blank" href={social.link} rel='noopener noreferrer' className='text-white-800 px-3 hover:text-blue-500 justify-center rounded-full glow p-2'>
        ...
        <div data-aos='fade-down' className='mt-6'>
          ...
          <ul className='flex flex-wrap gap-2'>
            {researchInterests.map((interest, index) => (
              <li key={index} className='bg-blue-400 dark:bg-blue-900 text-blue-800 dark:text-blue-100 px-3 py-1 rounded-full text-xs font-medium'>
                {interest}
              </li>
            ))}
          </ul>
      ...
      <img 
        ...
        className='rounded-full border-2 p-1 border-white-500 img_glow' 
      />   
    </section>
  );
}
